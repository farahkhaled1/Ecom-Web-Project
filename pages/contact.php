...
$username = $email = $linkk = $country =  "";
$username_err = $email_err = $linkk_err = $country_err =  "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["linkk"]))) {
        $linkk_err = "Please enter an link";
    } else {
        $linkk = trim($_POST["linkk"]);
    }
    ...
    if (empty($username_err) && empty($email_err) && empty($linkk_err) && empty($country_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_username, $param_email, $param_linkk, $param_country);
...
<body>
    ...
    <div class="wrapper">
        ...
        <form enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" autocomplete="off">
            ...
            <div class="form-group">
                <label>Link</label>
                <input type="text" name="linkk" class="form-control <?php echo (!empty($linkk_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $linkk; ?>">
